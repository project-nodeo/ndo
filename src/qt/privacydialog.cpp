                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNDO ought to be enough for anybody." - Bill Gates, 2017
    ui->zNDOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNDOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNDO->setEnabled(false);
        ui->pushButtonMintzNDO->setToolTip(tr("zNDO is currently disabled due to maintenance."));

        ui->pushButtonSpendzNDO->setEnabled(false);
        ui->pushButtonSpendzNDO->setToolTip(tr("zNDO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNDOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNDO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNDO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNDO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNDO in ") +
...
void PrivacyDialog::on_pushButtonSpendzNDO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNDO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNDO
        sendzNDO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNDO
    sendzNDO();
}

void PrivacyDialog::on_pushButtonZNodeoControl_clicked()
{
    ZNodeoControlDialog* zNDOControl = new ZNodeoControlDialog(this);
    zNDOControl->setModel(walletModel);
    zNDOControl->exec();
}

void PrivacyDialog::setZNodeoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNDOSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNDO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Nodeo Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNDOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNDO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NODEO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNDO</b>";
    ...
    // use mints from zNDO selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNodeoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNodeoControlDialog::GetSelectedMints();
    }
    ...
    // Clear znodeo selector in case it was used
    ZNodeoControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNDO Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Nodeo, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNDO Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNDO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNDO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNDO "));
    ui->labelzNDOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNDOSyncStatus->setVisible(fShow);
}
