...
#include "ui_znodeocontroldialog.h"
...
std::list<std::string> ZNodeoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNodeoControlDialog::listMints;

ZNodeoControlDialog::ZNodeoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNodeoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNodeoControlDialog::~ZNodeoControlDialog()
{
    delete ui;
}

void ZNodeoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNodeoControlDialog::updateList()
{
...
void ZNodeoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNodeoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNodeo_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNodeoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNodeoControlDialog::GetSelectedMints()
{
...
void ZNodeoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
